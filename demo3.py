...
class ERPApp:
    ...
    def buscar(self):
        busqueda = self.busqueda_entry.get()

        # Buscar en la tabla de productos
        query_productos = "SELECT * FROM productos WHERE nombre LIKE %s"
        values_productos = ('%' + busqueda + '%',)
        resultados_productos = self.db_connector.execute_select_query(query_productos, values_productos)

        # Buscar en la tabla de finanzas
        query_finanzas = "SELECT * FROM finanzas WHERE nombre LIKE %s"
        values_finanzas = ('%' + busqueda + '%',)
        resultados_finanzas = self.db_connector.execute_select_query(query_finanzas, values_finanzas)

        # Buscar en la tabla de personal
        query_personal = "SELECT * FROM personal WHERE nombre LIKE %s"
        values_personal = ('%' + busqueda + '%',)
        resultados_personal = self.db_connector.execute_select_query(query_personal, values_personal)
        ...
        for producto in resultados_productos:
            self.resultados_text.insert(tk.END, f"ID: {producto[0]}, Nombre: {producto[1]}, Cantidad: {producto[2]}\n")

        self.resultados_text.insert(tk.END, "\nFinanzas:\n")
        for finanza in resultados_finanzas:
            self.resultados_text.insert(tk.END, f"ID: {finanza[0]}, Concepto: {finanza[1]}, Monto: {finanza[2]}\n")

        self.resultados_text.insert(tk.END, "\nPersonal:\n")
        for persona in resultados_personal:
            self.resultados_text.insert(tk.END, f"ID: {persona[0]}, Nombre: {persona[1]}, Acción: {persona[2]}\n")
